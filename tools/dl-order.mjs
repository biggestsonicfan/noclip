 ...
 *
 * Run:  node tools/dl-order.mjs <capture-prefix> [modelA] [modelB] [rom.zip]
 *       node tools/dl-order.mjs cap/south 517 555
 */
...
if (!PREFIX) {
    console.error('usage: node tools/dl-order.mjs <capture-prefix> [modelA] [modelB] [rom.zip]');
    process.exit(2);
}
...
for (const ext of ['.bin', '.json']) {
    if (!fs.existsSync(PREFIX + ext)) {
        ...
        console.error('Take one with tools/mame-capture-dl.py — see tools/README.md.');
        process.exit(2);
    }
}
