 ...
 *   node tools/extract-texram.mjs --rom /c/m2/2d/sfight.zip --out /tmp/tex
 *
 * Writes texram0.bin, texram1.bin, lumaram.bin and colorxlat.bin — the four
 * files `tools/dump-atlas.mjs` reads and the viewer's Textures panel accepts.
 *
 ...
 * A real capture comes from MAME — see tools/texram.md.
 */
import fs from 'node:fs';
import path from 'node:path';
import { loadRomSet } from '../js/romset.js';
import { readStageTable } from '../js/stages.js';
import { buildTexram } from '../js/texture.js';
import { buildLumaram, buildColorxlat } from '../js/colors.js';
...
fs.writeFileSync(path.join(outDir, PROVENANCE),
    `Built by tools/extract-texram.mjs from ${path.basename(romPath)}.\n` +
    ...
    'not captured from hardware. Do not build tools/texram-ref.json from them: the\n' +
