...
/* ---- run it ---------------------------------------------------------------
 *
 * node tools/i960dis.mjs <hex address> [hex length] [rom.zip]
 *
...
if (import.meta.filename === process.argv[1]) {
    ...
    const { loadRomSet } = await import('../js/romset.js');
    const { readStageTable } = await import('../js/stages.js');

    const [addrArg, lenArg, romArg] = process.argv.slice(2);
    if (!addrArg) {
        console.error('usage: node tools/i960dis.mjs <hex address> [hex length] [rom.zip]');
        process.exit(2);
    }
