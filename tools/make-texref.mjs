 ...
 *
 * Run once against a MAME capture — see tools/texram.md for how to take one —
 * and the result, `tools/texram-ref.json`, is what `test-texram.mjs` and
 * `test-colors.mjs` hold the ported routines to from then on. The capture
 * itself is then not needed and is not tracked.
 *
 *   node tools/make-texref.mjs [--in texram] [--rom sfight.zip]
 *
...
import { SHEET_BYTES } from '../js/texture.js';
import { LUMA_BYTES, CXLAT_BYTES, PALETTE_LUMA0 } from '../js/colors.js';
...
const OUT = 'tools/texram-ref.json';
...
const readExact = (p, want) => {
    if (!fs.existsSync(p)) {
        console.error(`missing ${p} — see tools/texram.md for how to capture one`);
        process.exit(2);
    }
...
if (fs.existsSync(path.join(inDir, PROVENANCE))) {
    console.error(inDir + ' was built by tools/extract-texram.mjs, not captured from' +
...
const manifest = {
    note: 'SHA-256 over a MAME capture of the real board, cut at the slices ' +
        ...
        'tautologies. See tools/texref.mjs.',
