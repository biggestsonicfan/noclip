 ...
 *
 * `tools/test-motion-mame.mjs` checks everything the command stream spells out
 ...
 *
 * `tools/dl-rig.mjs` rebuilds those two matrices out of the stream: the body
 ...
 *
 * Run: node tools/test-head-mame.mjs <capture-prefix> <char> [rom.zip]
 */
...
import { readCharacter } from '../js/characters.js';
import { decodeMotion, sampleMotion } from '../js/motion.js';
import { buildPose } from '../js/pose.js';

const [prefix, charArg, romArg] = process.argv.slice(2);
if (!prefix) {
    console.log('usage: node tools/test-head-mame.mjs <capture-prefix> <char> [rom.zip]');
    process.exit(2);
}
const read = (p) => {
