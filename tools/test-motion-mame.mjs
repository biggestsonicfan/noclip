 ...
 *
 * `tools/mame-motion.py` records every word the i960 writes to the coprocessor
 ...
 *
 * Run: node tools/test-motion-mame.mjs [capture-prefix|csv] [rom.zip] [--save]
 */
...
if (SOURCE.endsWith('.csv')) {
    if (!fs.existsSync(SOURCE)) {
        console.log(`no ${path.basename(SOURCE)} — run tools/mame-motion.py, then this with --save`);
        process.exit(0);
    }
    rows = readCsv(SOURCE);
