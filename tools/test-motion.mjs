 ...
 *
 * Run: node tools/test-motion.mjs [rom.zip]
 */

import fs from 'fs';
import { loadRomSet } from '../js/romset.js';
import { readCharacter, CHARACTERS, faceVariantModels } from '../js/characters.js';
import {
    decodeMotion, sampleMotion, listMotions, evalChannel,
    MOTION_COUNT, LINEAR, HERMITE, TANGENT_RATE,
} from '../js/motion.js';
import { buildPose, poseMatrices, turnedBy, SLOT_COUNT, SLOT_PARENT } from '../js/pose.js';
import { decodeModel } from '../js/model.js';
import { readOsage, osageParts } from '../js/osage.js';
import { readTails, tailParts, CYCLE_LENGTH, LEAD as TAILS_LEAD } from '../js/tails.js';
import {
 ...
 *
 * `tools/mame-osage.py` breakpoints `os_set_osage` in a real match and reads
 ...
 */
{
    const cap = 'tools/osage-fang-segments.json';
 ...
 * short routines in the program ROM, and `tools/i960dis.mjs` reads it, so the
