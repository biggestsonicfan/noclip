...
```
node tools/extract-texram.mjs                          # South Island, into the temp dir
node tools/extract-texram.mjs --stage 5 --fighter 0
node tools/extract-texram.mjs --rom path/to/sfight.zip --out /tmp/tex
```

That writes `texram0.bin`, `texram1.bin`, `lumaram.bin` and `colorxlat.bin` —
what `tools/dump-atlas.mjs` reads (pass the same directory as its second
...
is why `tools/make-texref.mjs` refuses to build a reference from them.

## Checking the port against real hardware

`tools/test-texram.mjs` and `tools/test-colors.mjs` hold the ported routines
against a MAME capture of the actual board. The capture is not here; what is
tracked is `tools/texram-ref.json`, SHA-256 over it, cut at the slices those
...
`node tools/make-texref.mjs --in <dir>`.

## Taking a capture

`tools/mame-stage-capture.lua` forces a stage and dumps once texture RAM
settles; `tools/mame-dump-texram.lua` with `TEXRAM_KEY=KEYCODE_F12` dumps
...
mame.exe sfight -rompath <dir-with-only-zips> \
  -autoboot_script tools/mame-dump-texram.lua
```
